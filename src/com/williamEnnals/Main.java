...
public class Main {
        ...
        while (active) {
            ...
            switch (choice) {
                ...
                case "q":
                    ...
                    break;
                case "oc":
                    user.openAccount(AccountTypes.CHECKINGS);
                    break;
                case "os":
                    user.openAccount(AccountTypes.SAVINGS);
                    break;
                case "cc":
                    user.openAccount(AccountTypes.CREDITCARD);
                    break;
                case "ocwd":
                    double deposit = captureDeposit();
                    user.openAccount(AccountTypes.CHECKINGS, deposit);
                    break;
                case "oswd":
                    deposit = captureDeposit();
                    user.openAccount(AccountTypes.SAVINGS, deposit);
                    break;
                case "vb":
                    user.viewAccounts();
                    break;
                case "va":
                    ...
                    user.viewAccountBalance(accountNumber);
                case "d":
                    deposit = captureDeposit();
                    user.getAccountNumbers();
                    accountNumber = captureAccountNumber();
                    user.depositFunds(accountNumber, deposit);
                    break;
                case "w":
                    deposit = captureDeposit();
                    user.getAccountNumbers();
                    accountNumber = captureAccountNumber();
                    user.withdrawFunds(accountNumber, deposit);
                    break;
                case "t":
                    user.getAccountNumbers();
                    List<Long> accountList = captureAccountNumber(2);
                    double amount = captureDeposit();
                    user.transferFunds(accountList, amount);
                    break;
                default:
    ...
    public static User createNewUser() {
        ...
        return new User(firstName, lastName, userName, age, social);
    }
